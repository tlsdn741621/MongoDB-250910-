...
// 순서2) price 별로 내림 차순 정렬, 예를 들어 높은 가격대에서, 낮은 가격대로 정렬을 110 만건정렬
// 검색 조건 1) lt미만, 3000 미만, 가격이 높은 순에서 부터 찾기 시작하면, 정렬이
// 검색 조건 2) gt미만, 3000 초과, 가격이 높은 순에서 부터 찾기 시작하면, 정렬이, 효율적이다.

var start = new Date();
db.products.find({ category: "Electronics", price: {$gt : 3000} }).explain("executionStats");
var end = new Date();
print("복합 인덱스 가격 추가 검색 시간: " + (end - start) + "ms");
